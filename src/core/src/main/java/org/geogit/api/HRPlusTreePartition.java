...
/**
 * Divides a list of nodes spatially. Constructor accepts a list of nodes and then chooses the
 * optimal point at which to divide them into two groups. Tracks partition details, storing them in
 * fields.
 * <p>
 * Based on the algorithm from {@link http
 * ://dbs.mathematik.uni-marburg.de/publications/myPapers/1990/BKSS90.pdf} Create a spatial
 * partition of a list of nodes, minimizing the overlap between envelopes and the total area of
 * envelopes bounding the partitions. These metrics were explicitly stated and experimentally tested
 * in the paper.
 */
public class HRPlusTreePartition extends HRPlusTreeUtils {
    ...
    /**
     * Accept a list of nodes sorted by position along a one-dimensional axis. Create a partition of
     * these nodes, split at one point along the axis.
     * <p>
     * The paper by Beckmann et. al [2] recommends a 'goodness' measure minimizing overlap with area
     * as the tiebreaker.
     * <p>
     * Record the overlap between the two segments of the partition, the total area covered by the
     * nodes in the partition, and the index of the list at which nodes are divided.
     * <p>
     * Should only be called on a group of nodes that overflowed the container. (Otherwise, we have
     * no meaningful way to divide nodes into subgroups.) The {@code Preconditions} check asserts
     * this invariant.
     * <p>
     ...
     * <ul>
     * <p>
     * We pick the best distribution of these M+1 nodes by iterating over (M - 2m+1) distributions
     * and choosing the one with the best 'goodness' rating. The kth distribution has the first
     ...
     * <p>
     * Determines how much area is required to cover both partitions in total. Value is the sum of
     * the area of the bounding box enclosing the first with the area of the box enclosing the
     * second.
     * 
     ...
     * <p>
     * The split point is the index at which to take a subList to divide the list of nodes (given as
     * input to the constructor) according to this partition. Equal to the number of nodes in the
     * first group.
     * 
