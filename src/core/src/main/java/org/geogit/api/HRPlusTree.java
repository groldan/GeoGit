...
/**
 * Implementation of the HRPlus tree described in {@link http
 * ://www.cs.ust.hk/faculty/dimitris/PAPERS/ssdbm01.pdf} We chose the HRPlus tree for the spatial
 * index because it provides:
 * <ul>
 ...
 * <p>
 * <h4>Implementation Notes TODO build HR trees for different layers. Currently only have one tree
 * for one layer/feature.
 * <p>
 * TODO add data sharing between versions. An important part of HR+ trees is that data for old
 * versions may appear under the root of a new version these nodes are invisible to queries made on
 * the newer version, but exist nonetheless to save space. However this complicates lookups and
 * splits so we implement the more space-inefficient technique of keeping all versions separate
 * first.
 * 
 */
public class HRPlusTree extends HRPlusTreeUtils {
    ...
    /**
     * Connection to geogit database. TODO never initialized! We don't know how to connect.
     */
    ...
    /**
     * Initializes a HR+ tree of consisting of nodes of the given feature type in the revTree
     * 
     * @param revTree RevTree to build this HRPlusTree from
     * @param featureType type of feature we want to extract from
     * @param revTree . Other features are ignored.
     */
    ...
    /**
     * Insert a node into this HR+ tree. Finds the correct container node to insert into (@method
     * chooseSubtree), performs the insert, checking for overflow, rebalances envelopes,
     * simultaneously adding and re-placing nodes in the case of overflow.
     * 
    ...
    /**
     * Bounding box query limited to one version of the tree. Similar to {@method query},
     * but search is limited to one version of the tree. Returns null if
     * 
     * @param versionId does not appear as a key in
     * @param rootMap .
     * 
    ...
    /**
     * Add a new root (new versionid) to this tree's root map. Either add to an existing entry or
     * create a new one.
     * 
     * @param newRoot The node to insert into {@field rootTable}. This roots versionId must
     *        not already appear in the tree
     */
    ...
    /**
     * Update an entry of this tree's root map. We basically need to modify an existing version tree
     * of a root-map. Do this by deleting it from the root-map and inserting the new root.
     * 
     * @param root new root to add
     * @param versionId version of the new root
     */
    private void replaceRootTableEntry(HRPlusContainerNode root, ObjectId versionId,
            HRPlusContainerNode newRoot) {
     ...
     * <p>
     * TODO: do a version split. This comes after we share nodes between versions of the tree. For
     * now, the split is only spatial.
     * 
    ...
    /**
     * Spatially divide one container node. Minimize the perimeter/margin and overlap of subsets of
     * nodes. (This heuristic helps eliminate search paths during queries.)
     * <p>
     * Algorithm computes and compares the 'goodness' of margin (perimeter) values. See @method
     * sumOfMargins for details.
     * 
    ...
    public HRPlusContainerNode keySplitContainerNode(HRPlusContainerNode containerNode) {
        ...
        Preconditions.checkArgument(containerNode != null
        ...
        // New container has same versionId as old one, for now. (Should maybe get a brand new
        // timestamp.)
        // The newContainerNode is currently disconnected from the tree.
        // This is fixed in {@code HRPlusTree#adjustEnvelopes}
        HRPlusContainerNode newContainerNode = new HRPlusContainerNode(null, containerNode
                .getVersionIds().get(0));
    ...
    /**
     * Determine whether a container is a root. Assumes the container is already part of the tree.
     * 
     * @param containerNode
     * @return true if
     * @param containerNode is contained in {@field rootMap}
     */
    ...
    /**
     * Re-distribute a tree's nodes among roots. This may happen after a regular insert or after an
     * insert where an old container node was split.
     * 
     ...
     */
    private void adjustTree(HRPlusContainerNode containerNode, HRPlusContainerNode newContainerNode) {
        HRPlusNode parentNode;
        if (!this.isRoot(containerNode)) {
            parentNode = containerNode.getParentNode();
            HRPlusContainerNode parentContainer = parentNode.getParentContainer();
            Envelope containerMBR = containerNode.getMBR();
            parentNode.setBounds(containerMBR);

            if (newContainerNode != null) {
                HRPlusNode newNode = new HRPlusNode(newContainerNode.getMBR(), newContainerNode
                        .getVersionIds().get(0));
            ...
            if (parentContainer.getNumNodes() > this.getMaxDegree()) {
                ...
                splitParent = treatOverflow(parentContainer, parentContainer.getVersionIds().get(0));
            }
            ...
            HRPlusNode newNode1 = new HRPlusNode(containerNode.getMBR(), containerNode
                    .getVersionIds().get(0));
            HRPlusNode newNode2 = new HRPlusNode(newContainerNode.getMBR(), newContainerNode
                    .getVersionIds().get(0));
            ...
            this.replaceRootTableEntry(containerNode, containerNode.getVersionIds().get(0), newRoot);
        }
    }
     ...
     */
    private HRPlusContainerNode chooseSubtree(final HRPlusNode newNode, ObjectId versionId) {
    ...
    /**
     * Recursive helper method for {@code chooseSubtree}. Follows the R* algorithm as discussed in:
     * {@link http://infolab.usc.edu/csci587/Fall2011/papers/p322-beckmann.pdf}
     * 
     * @param newNode Node we want to find a container for
     * @param container Current container node we are searching
     * @return a leaf container node that is the best fit
     */
    private HRPlusContainerNode helpChooseContainer(HRPlusContainerNode containerNode,
            HRPlusNode newNode) {
    ...
    /**
     * Get all nodes contained within the root. Does not search recursively! Only gets the nodes
     * whose parent is the root.
     * 
     * @param container The root container to search
     * @return list of nodes with
     * @param container as parent.
     */
    ...
    /**
     * Gets all nodes associated with a version id. For now, this is all the nodes in the tree
     * (because each tree has just one version id). Use this for testing.
     * 
