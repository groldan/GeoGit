...
/**
 * The core building block of HR+ Trees. A container node surrounds data envelopes, representing
 * them by their minimum bounding rectangle (MBR). Nodes within a container should be close,
 * spatially, and every sub-container of this container should have an MBR contained within this
 * MBR.
 * <p>
 * We use containers rather than pure nodes as the basic unit within an HR+ Tree to track overflow.
 * Ideally, data will be spread evenly throughout the data structure; containers ensure this
 * property. If, during insert, we add too many nodes to one container, the container is split into
 * two and its nodes repartitioned. The method {@link HRPlusTree#insert} performs the check and
 * repartitioning.
 * 
 * <h4>Notes</h4>
 * <ul>
...
public class HRPlusContainerNode implements RevObject {
    ...
    /**
     * Map of nodes inhabiting this container. Keys are the unique {@code ObjectId} for the nodes.
     */
    private List<HRPlusNode> childNodes = new ArrayList<HRPlusNode>();

    /**
     * Unique id describing this container. TODO never set
     */
    private ObjectId objectId;

    /**
     * Parent of this container. Will be null if this container is a root.
     */
    ...
    /**
     * Counts the number of nodes in this container. Does not count nodes within sub-containers.
     * 
    ...
    /**
     * Adds a node to this container. Simply inserts the node into {@code nodeMap}. If the objectId
     * of {@code node} is identical to an id existing in {@code nodeMap}, erase the existing node.
     *
    ...
    /**
     * Removes a node from this container. Specifically, removes the node whose object id matches
     * {@code objectId}
     * 
     * @param objectId
    ...
    /**
     * Return all the leaf nodes under this container. If this is a leaf container return the nodes
     * it contains, otherwise, for each node that it contains, call recursively
     * 
    ...
    /**
     * Determine whether this container is a leaf. A leaf has no sub-containers (a leaf may contain
     * nodes).
     * <p>
     * An HR+ tree is balanced by construction, so if one node within this container has no
     * children, none of the nodes within this container do.
     * 
    ...
    /**
     * Check whether any children of this node are leaves. Used in {@link HRPlusTree#chooseSubtree}.
     * 
    ...
    /**
     * Compute the minimum bounding rectangle for nodes in this container. MBR is empty if the
     * container is empty. MBR implicitly covers all nodes in sub-containers; those nodes are not
    ...
    /**
     * Search this container for nodes within the argument envelope and recurse into their
     * containers.
     * 
     * @param env
