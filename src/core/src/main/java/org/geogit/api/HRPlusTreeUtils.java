...
/**
 * Base class for HRPlus tree implementation of the spatial index. This class contains utility
 * methods for manipulating HRPlus trees.
 * <p>
 * Utilities are factored out because we distinguish between a plain HRPlus tree and an HRPlus tree
 * partition. The latter is used to speed up insertions.
 * <p>
 * For the original idea, see [1] {@link http://www.cs.ust.hk/faculty/dimitris/PAPERS/ssdbm01.pdf}
 * and for details on insertion, see Section 4 of: [2] {@link http
 * ://dbs.mathematik.uni-marburg.de/publications/myPapers/1990/BKSS90.pdf}
 */
public class HRPlusTreeUtils {
    ...
    /**
     * Create an envelope covering all nodes in param list. Used to create a container node for the
     * HRPlus tree.
     * 
     ...
     */
    protected static double getOverlap(List<HRPlusNode> firstGroup, List<HRPlusNode> secondGroup) {
    ...
    /**
     * Find how much the area of a container holding
     * 
     * @param nodeA will increase if
     * @param nodeB is added to it.
     ...
     */
    protected static double getTotalAreaOfTwoRegions(List<HRPlusNode> firstGroup,
     ...
     */
    protected static double getTotalMarginOfTwoRegions(List<HRPlusNode> firstGroup,
            List<HRPlusNode> secondGroup) {
    ...
    /**
     * The paper [2] suggests minimizing perimeters as a useful way to optimize an index. Sum the
     * perimeters of all possible partitions of nodes along one axis.
     * <p>
     * This method iterates of the valid ways of partitioning
     * 
     * @param nodes and calculates the total perimeter of each partition. Returns the sum of all
     *        these total perimeters.
     * 
    ...
    protected static double sumOfMargins(List<HRPlusNode> nodes) {
        if (nodes.isEmpty() || nodes.size() == 1) {
            ...
            while (!secondGroup.isEmpty()) {
                marginValueSum += marginOf(boundingBoxOf(firstGroup))
                        + marginOf(boundingBoxOf(secondGroup));
     ...
     * <p>
     * Take a list of nodes, sorted by position on a one-dimensional axis. Return the subset of
     * these nodes that is closest together based on the one-dimensional axis and the total
     * perimeter and area of their enclosing envelope.
     * 
    ...
    protected static List<HRPlusNode> partitionByMinOverlap(List<HRPlusNode> minSort,
        ...
        if (overlapMinSort < overlapMaxSort
                || (overlapMinSort == overlapMaxSort && areaValueMinSort <= areaValueMaxSort)) {
