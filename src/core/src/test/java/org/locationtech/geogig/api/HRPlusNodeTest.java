...
public class HRPlusNodeTest {
    ...
    public void testQueryNodeSuccess() {
        // Set up children.
        HRPlusNode childA = new HRPlusNode(ObjectId.forString("node 1"), new Envelope(-2, -3, -2,
                -3), ObjectId.forString("v1"));
        HRPlusNode childB = new HRPlusNode(ObjectId.forString("node 2"), new Envelope(2, 3, 2, 3),
                ObjectId.forString("v1"));
        HRPlusNode childC = new HRPlusNode(ObjectId.forString("node 3"),
                new Envelope(-2, -3, 2, 3), ObjectId.forString("v1"));
        ...
        // Set up parent.
        HRPlusNode node = new HRPlusNode(ObjectId.forString("parent node"), new Envelope(2, 3, -2,
                -3), ObjectId.forString("v1"));
        node.setChild(child);
        // Execute query
        List<HRPlusNode> matches = new ArrayList<HRPlusNode>();
