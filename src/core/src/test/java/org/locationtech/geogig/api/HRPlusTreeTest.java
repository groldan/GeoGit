...
/**
 * Class for testing the HRPlusTree. Note that all nodes in an HRPlusTree belong to the same feature
 * type, for now.
 */
public class HRPlusTreeTest {
    ...
    public void testGetNodes() {
        /*
         * Create a tree with 3 features (Same Version). Tests the following: 1. getNodes() -
         * Returns all nodes of the tree after insertion. Check if this contains all the nodes we
         * actually inserted. 2. Size of expected result and actual result
         */
    ...
    public void testGetNumRoots() {
        /*
         * Tests the no. of roots(entry-points) in the tree. This should be equal to the no. of
         * versions the nodes come from.
         */
        HRPlusTree tree = new HRPlusTree();
        // 4 feature nodes
    ...
    public void testInsertOneOverflow() {
        ...
        for (HRPlusNode node : leaves) {
            if (!foundA) {
                foundA = envA.equals(node.getBounds());
            }
            if (!foundB) {
                foundB = envB.equals(node.getBounds());
            }
            if (!foundC) {
                foundC = envC.equals(node.getBounds());
            }
            if (!foundD) {
                foundD = envD.equals(node.getBounds());
            }
        }
        assertTrue(foundA);
        assertTrue(foundB);
    ...
    public void testInsertFiveNonOverlappingNodes() {
        ...
        for (HRPlusNode node : leaves) {
            ...
            if (!foundE) {
                foundE = envE.equals(node.getBounds());
            }
            if (!foundF) {
                foundF = envF.equals(node.getBounds());
            }
        }
        assertTrue(foundA);
        assertTrue(foundB);
    ...
    public void testOverflowTwelveInserts() {
        /*
         * Keep adding nodes belonging to the same version which are very close to each other. Check
         * the no. of containers for the root.
         */
        HRPlusTree tree = new HRPlusTree();
        ObjectId versionId1 = ObjectId.forString("Version1");
    ...
    public void testQueries() {
        /*
         * Inserts certain no. of non-overlapping nodes belonging to same version. Tests the
         * following queries: 1. A large bbox query - should return all the nodes. 2. Query (BBox of
         * Node1) - should return node 1 3. Query (BBox of Node2) - should return node 2 4. Query
         * (BBox of Node3) - should return node 3
         */
        HRPlusTree tree = new HRPlusTree();
        ObjectId versionId1 = ObjectId.forString("Version1");
        Envelope envelopes[] = new Envelope[3];
    ...
    public void testQueries2() {
        /*
         * Inserts 4 non-overlapping nodes. One in each quadrant. | __ | __ |__| | |__|
         * --------|--------- __ | __ |__| | |__|
         * 
         * Tests the following queries: 1. A large bbox query - should return all the nodes. 2.
         * Query (BBox of Node1) - should return node 1 3. Query (BBox of Node2) - should return
         * node 2 4. Query (BBox of Node3) - should return node 3
         */
        HRPlusTree tree = new HRPlusTree();
        ObjectId versionId1 = ObjectId.forString("Version1");
    ...
    public void testOverflow16nodes() {
        /*
         * Inserts 16 non-overlapping nodes, 4 in each quadrant in the following order | __ __ | __
         * __ |_8| |_6| | |2_| |4_| __ __ | __ __ |_7| |_5| | |1_| |3_| | -----------------------
         * |------------------ __ __ | __ __ |11| |_9| | |13| |15| __ __ | __ __ |12| |10| | |14|
         * |16|
         */
        HRPlusTree tree = new HRPlusTree();
    ...
    public void testKeySplitContainerNodeTooManyNodes() {
        HRPlusNode nodeA = new HRPlusNode(ObjectId.forString("A"), new Envelope(0, 1, 0, 1),
                new ObjectId());
        HRPlusNode nodeB = new HRPlusNode(ObjectId.forString("B"), new Envelope(0, 1, 0, 1),
                new ObjectId());
        HRPlusNode nodeC = new HRPlusNode(ObjectId.forString("C"), new Envelope(0, 1, 0, 1),
                new ObjectId());
        HRPlusNode nodeD = new HRPlusNode(ObjectId.forString("D"), new Envelope(0, 1, 0, 1),
                new ObjectId());
        HRPlusNode nodeE = new HRPlusNode(ObjectId.forString("E"), new Envelope(0, 1, 0, 1),
                new ObjectId());
    ...
    public void testKeySplitContainerNodeNonIntersectingSquare() {
        HRPlusNode nodeA = new HRPlusNode(ObjectId.forString("A"), new Envelope(1, 2, 1, 2),
                new ObjectId());
        HRPlusNode nodeB = new HRPlusNode(ObjectId.forString("B"), new Envelope(-1, -2, 1, 2),
                new ObjectId());
        HRPlusNode nodeC = new HRPlusNode(ObjectId.forString("C"), new Envelope(-1, -2, -1, -2),
                new ObjectId());
        HRPlusNode nodeD = new HRPlusNode(ObjectId.forString("D"), new Envelope(1, 2, -1, -2),
                new ObjectId());
    ...
    public void testKeySplitContainerNodeNonIntersectingLine() {
        HRPlusNode nodeA = new HRPlusNode(ObjectId.forString("A"), new Envelope(1, 2, 1, 2),
                new ObjectId());
        HRPlusNode nodeB = new HRPlusNode(ObjectId.forString("B"), new Envelope(3, 4, 1, 2),
                new ObjectId());
        HRPlusNode nodeC = new HRPlusNode(ObjectId.forString("C"), new Envelope(5, 6, 1, 2),
                new ObjectId());
        HRPlusNode nodeD = new HRPlusNode(ObjectId.forString("D"), new Envelope(7, 8, 1, 2),
                new ObjectId());
    ...
    public void testKeySplitContainerNodeParallelOverlapOne() {
        HRPlusNode nodeA = new HRPlusNode(ObjectId.forString("A"), new Envelope(-10, -4, -10, 10),
                new ObjectId());
        HRPlusNode nodeB = new HRPlusNode(ObjectId.forString("B"), new Envelope(-3, 3, -10, 10),
                new ObjectId());
        HRPlusNode nodeC = new HRPlusNode(ObjectId.forString("C"), new Envelope(4, 10, -10, 10),
                new ObjectId());
        HRPlusNode nodeD = new HRPlusNode(ObjectId.forString("D"), new Envelope(-11, -5, 8, 10),
                new ObjectId());
    ...
    public void testKeySplitContainerNodeParallelOverlapTwo() {
        ...
        HRPlusNode nodeC = new HRPlusNode(ObjectId.forString("C"), new Envelope(4, 10, -10, 10),
                new ObjectId());
        HRPlusNode nodeD = new HRPlusNode(ObjectId.forString("D"), new Envelope(-11, 4, 8, 10),
                new ObjectId());
    ...
    public void testKeySplitContainerNodeParallelOverlapThree() {
        ...
        HRPlusNode nodeC = new HRPlusNode(ObjectId.forString("C"), new Envelope(4, 10, -10, 10),
                new ObjectId());
        HRPlusNode nodeD = new HRPlusNode(ObjectId.forString("D"), new Envelope(-11, 11, 8, 10),
                new ObjectId());
    ...
    public void testKeySplitContainerNodePairsOfIntersectingSquares() {
        HRPlusNode nodeA = new HRPlusNode(ObjectId.forString("A"), new Envelope(-10, -8, 6, 8),
                new ObjectId());
        HRPlusNode nodeB = new HRPlusNode(ObjectId.forString("B"), new Envelope(-9, -7, 5, 7),
                new ObjectId());
        HRPlusNode nodeC = new HRPlusNode(ObjectId.forString("C"), new Envelope(10, 8, -6, -8),
                new ObjectId());
        HRPlusNode nodeD = new HRPlusNode(ObjectId.forString("D"), new Envelope(9, 7, -5, -7),
                new ObjectId());
